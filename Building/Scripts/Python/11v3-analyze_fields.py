...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 14 15:44:18 2025
...
def main():
    ...
    # CSV export of fit results
    import csv
    with open("fit_results.csv", "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Pair", "Amplitude", "Center (MV/cm)", "Sigma (MV/cm)", "Sigma/sqrt(2) (MV/cm)"])
        writer.writerow([f"{atom[uv1_idx[0]-1]}-{atom[uv1_idx[1]-1]}", fitval1[0], fitval1[1], fitval1[2], fitval1[2]/np.sqrt(2)])
        writer.writerow([f"{atom[uv2_idx[0]-1]}-{atom[uv2_idx[1]-1]}", fitval2[0], fitval2[1], fitval2[2], fitval2[2]/np.sqrt(2)])

    print("Results saved to fit_results.csv and electric_field_histograms.png")
